...
func style(segmentedControl: UISegmentedControl, fontName: String, selectedColor: UIColor, unselectedColor: UIColor, dividerColor: UIColor) {
    ...
    let segBG = drawImage(ofSize: CGSize(width: 50,
                                         height: 30),
                          andColor: unselectedColor)
    let segSelectedBG = drawImage(ofSize: CGSize(width: 50,
                                                 height: 30),
                                  
                                  andColor: selectedColor)
    let segDividerImage = drawImage(ofSize: CGSize(width: 1,
                                                   height: 30),
                                    andColor: dividerColor)
    
    segmentedControl.setBackgroundImage(segBG,
                                        for: [],
                                        barMetrics: .default)
    segmentedControl.setBackgroundImage(segSelectedBG,
                                        for: .selected,
                                        barMetrics: .default)
    
    segmentedControl.setDividerImage(segDividerImage,
                                     forLeftSegmentState: [],
                                     rightSegmentState: [],
                                     barMetrics: .default)
    
    let shadow = NSShadow()
    shadow.shadowOffset = CGSize(width: 0,
                                 height: 0)
...
func waitScene(ofSize size: CGSize) -> UIView {
    let overlay = UIView(frame: CGRect(origin: CGPoint(x: 0,
                                                       y: 0),
                                       
                                       size: size))
    overlay.backgroundColor = UIColor(white: 0.2,
                                      alpha: 0.6)
